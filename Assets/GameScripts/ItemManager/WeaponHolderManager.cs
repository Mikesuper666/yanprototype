...
using System.Collections.Generic;
    ...
    [ClassHeader("Weapon Holder Manager", "Create a new empty object inside a bone and add the vWeaponHolder script")]
    public class WeaponHolderManager : mMonoBehaviour
    {
        public WeaponHolder[] holders = new WeaponHolder[0];
        [HideInInspector]
        public bool inEquip;
        [HideInInspector]
        public ItemManager itemManager;

        public Dictionary<string, List<WeaponHolder>> holderAreas = new Dictionary<string, List<WeaponHolder>>();
        protected float equipTime;

        void OnDrawGizmosSelected()
        {
            holders = GetComponentsInChildren<WeaponHolder>(true);
        }
        ...
        void Start()
	    {
            ...
            if (itemManager)
            {
                ...
                if (holders != null)
                {
                    foreach (WeaponHolder holder in holders)
                    {
                        if (!holderAreas.ContainsKey(holder.equipPointName))
                        {
                            holderAreas.Add(holder.equipPointName, new List<WeaponHolder>());
        ...
        public void EquipWeapon(EquipArea equipArea, Item item)
        {           
        ...
        public void UnequipWeapon(EquipArea equipArea, Item item)
        {
        ...
        IEnumerator UnequipRoutine(WeaponHolder holder, bool activeHolder, bool activeWeapon, bool immediat = false)
        {
        ...
        IEnumerator EquipRoutine(WeaponHolder holder,bool activeHolder, bool activeWeapon, bool immediat = false)
        {
            if (!immediat)
	            inEquip = true;
            holder.SetActiveHolder(activeHolder);
