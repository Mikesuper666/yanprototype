...
public class MeleeAttackObject : MonoBehaviour
{
    ...
    public virtual void OnHit(HitBox hitBox, Collider other)
    {
        ...
        if (canApplyDamage && !targetColliders[hitBox].Contains(other.gameObject) && (meleeManager != null && other.gameObject != meleeManager.gameObject))
        {
            var inDamage = false;
            var inRecoil = false;
            if (meleeManager == null) meleeManager = GetComponentInParent<MeleeManager>();
            //check if meleeManager exist and apply  his hitProperties  to this
            HitProperties _hitProperties = meleeManager.hitProperties;

            /// Damage Conditions
            if (((hitBox.triggerType & HitBoxType.Damage) != 0) && _hitProperties.hitDamageTags == null || _hitProperties.hitDamageTags.Count == 0)
                inDamage = true;
            else if (((hitBox.triggerType & HitBoxType.Damage) != 0) && _hitProperties.hitDamageTags.Contains(other.tag))
                inDamage = true;
            else   ///Recoil Conditions  
            if (((hitBox.triggerType & HitBoxType.Recoil) != 0) && (_hitProperties.hitRecoilLayer == (_hitProperties.hitRecoilLayer | (1 << other.gameObject.layer))))
                inRecoil = true;
