...
[CustomEditor(typeof(MeleeAttackControl))]
public class MeleeAttackControlEditor : Editor
{
    MeleeAttackControl attackControl;
    ...
    void OnEnable()
    {
        try
        {
            attackControl = (MeleeAttackControl)target;
        ...
        indexSelected = -1;
        isHuman = true;
        m_Logo = (Texture2D)Resources.Load("triggerIcon", typeof(Texture2D));
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        for (int i = 0; i < attackControl.bodyParts.Count; i++)
        {
            ...
            if (GUILayout.Button(attackControl.bodyParts[i], EditorStyles.miniButton))
            {
                ...
                else
                {
                    ...
                    try
                    {
                        oldBodyPart = Enum.Parse(typeof(HumanBones), oldBodyPart).ToString();
                        isHuman = true;
                    }
                    catch { isHuman = false; }
        ...
        {
            currentAttackType = attackControl.meleeAttackType;
            if (currentAttackType == AttackType.MeleeWeapon)
            {
    ...
    void AddBodyPart()
    {
        ...
        if (attackControl.meleeAttackType == AttackType.Unarmed)
        {
            ...
            if (isHuman)
            {
                try
                {
                    currentBodyPart = EditorGUILayout.EnumPopup("Body Part", (HumanBones)Enum.Parse(typeof(HumanBones), currentBodyPart)).ToString();
                }
    ...
    void EditBodyPart()
    {
        ...
        if (attackControl.meleeAttackType == AttackType.Unarmed)
        {
            ...
            if (isHuman)
            {
                try
                {
                    oldBodyPart = EditorGUILayout.EnumPopup("Body Part", (HumanBones)Enum.Parse(typeof(HumanBones), oldBodyPart)).ToString();
                }
