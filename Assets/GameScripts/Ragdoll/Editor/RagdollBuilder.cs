...
class RagdollBuilder : ScriptableWizard
{
    ...
    public bool deleteThisCharacterBones = false;
    public enum RagdollAvatarType
    {
        Human, Generic
    }
    public RagdollAvatarType avatarType = RagdollAvatarType.Human;
    [Header("--- Animator of cursorObject Character ---")]
    public Animator anime;
    [Header("--- Generic character template ---")]
    public RagdollGenericTemplate genericTemplate;
    [Header("--- Bone ---")]
        ...
        foreach (BoneInfo bone in bones)
        {
            if (bone.anchor)
            {
                if (map[bone.anchor] != null)
                {
                    ...
                    return String.Format("{0} and {1} may not be assigned to same bone", bone.name, oldBone.name);
                }
    ...
    [MenuItem("YanProject/Add Basic/Components/Ragdoll")]
    static void CreateWizard()
    {
        ScriptableWizard.DisplayWizard("Create Ragdoll", typeof(RagdollBuilder),"Add Ragdoll");
    }
    ...
    void PrepareBones()
    {
        ...
        {
            anime = Selection.activeGameObject.transform.GetComponent<Animator>();//get animator from the object selected (dont forgive apply one)
        }
        if (anime != null && avatarType == RagdollAvatarType.Human)
            GetHumanBones();//get if human
        else GetGenericBones();//get if generic
        ...
        AddMirroredJoint("Hips", leftHips, rightHips, "Root", worldRight, worldForward, -20, 70, 30, typeof(CapsuleCollider), .3f, 1.5f);
        AddMirroredJoint("Knee", leftKnee, rightKnee, "Hips", worldRight, worldForward, -80, 0, 0, typeof(CapsuleCollider), .25f, 1.5f);
    ...
    /// <summary>
    /// Adiciona um objeto vazio e dentro um audio source e também um Ragdoll Script
    /// </summary>
    void RagdollAudioSource()
    {
        if (Selection.activeGameObject != null)
        {
            var ragdollAudioSource = new GameObject("ragdollAudioSource");//add a empty object
            ragdollAudioSource.transform.SetParent(Selection.activeGameObject.transform);
            ragdollAudioSource.transform.localPosition = Vector3.zero;
            var sourceObj = new GameObject("collisionAudio", typeof(AudioSource));//add a audio source object
            ...
            var rag = Selection.activeGameObject.AddComponent<Ragdoll>();//add a Ragdoll script on selected object
            rag.collisionSource = sourceObj.GetComponent<AudioSource>();
        }
    }//add a audiosource to play ragdolls sound
    ...
    void GetHumanBones()
    {
        try
        {
            root = anime.GetBoneTransform(HumanBodyBones.Hips);

            leftHips = anime.GetBoneTransform(HumanBodyBones.LeftUpperLeg);
            leftKnee = anime.GetBoneTransform(HumanBodyBones.LeftLowerLeg);
            leftFoot = anime.GetBoneTransform(HumanBodyBones.LeftFoot);

            rightHips = anime.GetBoneTransform(HumanBodyBones.RightUpperLeg);
            rightKnee = anime.GetBoneTransform(HumanBodyBones.RightLowerLeg);
            rightFoot = anime.GetBoneTransform(HumanBodyBones.RightFoot);

            leftArm = anime.GetBoneTransform(HumanBodyBones.LeftUpperArm);
            leftElbow = anime.GetBoneTransform(HumanBodyBones.LeftLowerArm);

            rightArm = anime.GetBoneTransform(HumanBodyBones.RightUpperArm);
            rightElbow = anime.GetBoneTransform(HumanBodyBones.RightLowerArm);

            middleSpine = anime.GetBoneTransform(HumanBodyBones.Chest);

            head = anime.GetBoneTransform(HumanBodyBones.Head);
    ...
    public void Cleanup()
    {
        foreach (BoneInfo bone in bones)
        {
            ...
            {
                if (collider.transform != leftFoot && collider.transform != rightFoot)
                    DestroyImmediate(collider);
            }

            Component[] collisionMenssages = bone.anchor.GetComponentsInChildren(typeof(CollisionMessage));
            foreach (CollisionMessage collisionMenssage in collisionMenssages)
                DestroyImmediate(collisionMenssage);
        }
    }//remove all components necessary for Ragdoll
