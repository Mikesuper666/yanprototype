...
namespace Invector.vCharacterController.vActions
{
    ...
    [ClassHeader("GENERIC ACTION", "Use the vTriggerGenericAction to trigger a simple animation.", iconName = "triggerIcon")]
    public class GenericAction : ActionListener
    {
        #region Variables
        ...
        protected PlayerInputs tpInput;
        ...
        {
            tpInput = GetComponent<PlayerInputs>();           
        }
        ...
        {            
            ...
            if (!tpInput.cc.customAction)
            {
                ...
                transform.rotation = tpInput.cc.anime.rootRotation;
            }
            transform.position = tpInput.cc.anime.rootPosition;
        }
        ...
        public virtual bool actionConditions
        {
            get
            {
                return !(tpInput.cc.isJumping || tpInput.cc.actions || !canTriggerAction || isPlayingAnimation) && !tpInput.cc.anime.IsInTransition(0);
            }
        }
        ...
        protected virtual void AnimationBehaviour()
        {
            if (playingAnimation)
            {
                ...
                if (triggerAction.useTriggerRotation)
                {
                    ...
                    transform.rotation = Quaternion.Lerp(transform.rotation, triggerAction.transform.rotation, tpInput.cc.anime.GetCurrentAnimatorStateInfo(0).normalizedTime);
                }

                if (triggerAction.resetPlayerSettings && tpInput.cc.anime.GetCurrentAnimatorStateInfo(0).normalizedTime >= triggerAction.endExitTimeAnimation)
                {
        ...
        protected virtual void ApplyPlayerSettings()
        {
            ...
            if (triggerAction.disableGravity)
            {
                ...
                tpInput.cc.anime.SetBool("IsGrounded", true);        // also ground the character on the animator so that he won't float after finishes the climb animation
                tpInput.cc.anime.SetInteger("ActionState", 1);       // set actionState 1 to avoid falling transitions     
            }
        ...
        protected virtual void ResetPlayerSettings()
        {
            ...
            if(!playingAnimation || tpInput.cc.anime.GetCurrentAnimatorStateInfo(0).normalizedTime >= triggerAction.endExitTimeAnimation)
            {
                tpInput.cc.EnableGravityAndCollision(0f);             // enable again the gravity and collision
                tpInput.cc.anime.SetInteger("ActionState", 0);     // set actionState 1 to avoid falling transitions
            }
